...
double compute_emat() {
  ...
  long int total_accesses = count_writes + count_reads;
  long int tlb_misses = total_accesses - count_tlbhits;
  long int time_tlbhits = MEMORY_ACCESS_TIME * count_tlbhits;
  long int time_tlbmisses = MEMORY_ACCESS_TIME * tlb_misses * 2;
  long int time_pagefaults = DISK_ACCESS_TIME * count_pagefaults;

  return (double) (time_tlbhits + time_tlbmisses + time_pagefaults)/total_accesses;
}
