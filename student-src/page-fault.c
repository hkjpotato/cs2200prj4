...
pfn_t pagefault_handler(vpn_t request_vpn, int write) {
  ...
  /* !!This is like the frame table in the textbook */
  ...
  if (victim_pcb != NULL) {
    ...
    /* since victim-pcb is a pointer pointing to pcb_t type, thus we use '->' */ 
    if (victim_pcb -> pagetable[victim_vpn].dirty) {
      /* if this is a dirty page, it means it has been changed since loaded to memory. 
      Thus we have to send it to disk
      */ 
